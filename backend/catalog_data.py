...
Products with `detection_label = None` (mango, packaged snacks, etc.) aren't
COCO classes, so the camera can't auto-detect them — they still exist in the
catalog and can be added to the cart manually (the qty +/- controls). An
open-vocabulary detector (YOLO-World + CLIP) would lift this limit, but it
needs far more RAM than this deployment's hosting plan allows — it OOM-crashed
the container, so we're back on the lightweight COCO-only model. A production
rollout would fine-tune the detector on the client's actual packaged SKUs
instead — see README "Roadmap".
"""
...
SEED_PRODUCTS = [
    ...
    ("SKU-1016", "Aam (Mango)",         70,   "Produce",   None,       None),
    ...
    ("SKU-1012", "Lays Chips Packet",   100,  "Snacks",    None,       None),
    ("SKU-1013", "Milk Pack 1L",        190,  "Dairy",     None,       None),
    ("SKU-1014", "Biscuit Pack",        60,   "Bakery",    None,       None),
    ("SKU-1015", "Instant Noodles",     45,   "Snacks",    None,       None),
]
